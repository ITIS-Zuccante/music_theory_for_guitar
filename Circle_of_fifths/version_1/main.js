...
let supportsPointerEvents = function() {
  ...
  (function (window, document) {
      ...
      const _getDraggedRotateTo = current => {
          // 30 (deg) is a twelfth of 360 degrees = 1 chord (semitone)
...
function launchFullWindow(config) {
    ...
    let cfg = {
        ...
        open : {
            ...
            title : 'Full screen [F, F1]'
        },
        exit : {
            icon : 'icon-fullScreen-exit',
            extension : '-exit',
            label : 'Exit full screen',
            title : 'Exit full screen [F, F1, esc]'
        }
    };
    ...
    const _instantiateLaunchObj = launchObj => {
        ...
        const _getSvgString = toOpen => {
            const param = _getCfg('extension', toOpen).substring(1);
...

        {
            ...
            // Toggle if the f, or f1, key is pressed
            document.addEventListener('keydown', event => {
              if (event.key === 'F' || event.key === 'F1') {
                _toggleFullScreen();
              }
            });
        }
